...
class Color_picker_plus_ft extends EE_Fieldtype {
	...
	private function _theme_url()
	{
		if (! isset($this->cache['theme_url']))
		{
			...
			$this->cache['theme_url'] = $theme_folder_url.'color_picker_plus/';
		}
	 ...
	 /**
     * Include the CSS and JS, but only once
     */
    private function _include_css_js()
    {
        if (!$this->EE->session->cache(__CLASS__, 'color_picker_assets_loaded'))
		{
			$this->EE->cp->add_to_foot('<link rel="stylesheet" media="screen" href="' . $this->_theme_url() . 'css/jPicker-1.1.6.min.css" />');
			$this->EE->cp->add_to_foot('<link rel="stylesheet" media="screen" href="' . $this->_theme_url() . 'css/jPicker.css" />');
			...
			$this->EE->cp->add_to_foot('<script type="text/javascript" src="' . $this->_theme_url() . 'jpicker-1.1.6.min.js"></script>');
			
			$this->EE->session->set_cache(__CLASS__, 'color_picker_assets_loaded', TRUE);
		}
    }
	...
	function display_field($data)
	{
		...
		*/
		$imagesPath = $this->_theme_url() . 'images/';
